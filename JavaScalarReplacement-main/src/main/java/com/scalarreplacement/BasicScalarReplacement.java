...
import com.scalarreplacement.InterproceduralScalarVisitors.SignatureVisitor;
import com.scalarreplacement.InterproceduralScalarVisitors.CallVisitor;



public class BasicScalarReplacement {
    public static Map<String,String> localClassType = new HashMap<>();
    public static Map<String, List<String>> liveFields = new HashMap<>();
    ...
    public static Set<String> declaredScalars = new HashSet<>();


    public static void main(String[] args) {

        File src = new File("/home/rohit/Desktop/Java_Scalar_Replacement/JavaScalarReplacement-main/src/main/java/example/");
        ...
        try {
            ...
            typeSolver.add(new JarTypeSolver(new File("/home/rohit/Desktop/Java_Scalar_Replacement/JavaScalarReplacement-main/src/main/Jar_Files/jakarta.servlet-api-5.0.0.jar")));
            typeSolver.add(new JavaParserTypeSolver("/home/rohit/Desktop/Java_Scalar_Replacement/JavaScalarReplacement-main/src/main/Jar_Files/section3"));
            typeSolver.add(new JavaParserTypeSolver("/home/rohit/Desktop/Java_Scalar_Replacement/JavaScalarReplacement-main/src/main/Jar_Files/jgfutil"));
            typeSolver.add(new JarTypeSolver(new File("/home/rohit/Desktop/Java_Scalar_Replacement/JavaScalarReplacement-main/src/main/Jar_Files/javax.xml-1.3.4.jar")));
            Path projectRoot = FileSystems.getDefault().getPath("/home/rohit/Desktop/Java_Scalar_Replacement/JavaScalarReplacement-main/src/main/java/example/");
            typeSolver.add(new JavaParserTypeSolver("/home/rohit/Desktop/Java_Scalar_Replacement/JavaScalarReplacement-main/src/main/Jar_Files/h2/src/main"));
            typeSolver.add(new JarTypeSolver(new File("/home/rohit/Desktop/Java_Scalar_Replacement/JavaScalarReplacement-main/src/main/Jar_Files/javaparser-core-3.25.5.jar")));
            typeSolver.add(new JarTypeSolver(new File("/home/rohit/Desktop/Java_Scalar_Replacement/JavaScalarReplacement-main/src/main/Jar_Files/jts-core-1.17.0.jar")));
            typeSolver.add(new JarTypeSolver(new File("/home/rohit/Desktop/Java_Scalar_Replacement/JavaScalarReplacement-main/src/main/Jar_Files/jakarta.servlet-api-5.0.0.jar")));
            typeSolver.add(new JarTypeSolver(new File("/home/rohit/Desktop/Java_Scalar_Replacement/JavaScalarReplacement-main/src/main/Jar_Files/org.osgi.service.jdbc-1.0.0.jar")));
            typeSolver.add(new JarTypeSolver(new File("/home/rohit/Desktop/Java_Scalar_Replacement/JavaScalarReplacement-main/src/main/Jar_Files/lucene-core-8.5.2.jar")));
            typeSolver.add(new JarTypeSolver(new File("/home/rohit/Desktop/Java_Scalar_Replacement/JavaScalarReplacement-main/src/main/Jar_Files/rt-4.0.3.jar")));
...
//            File src1 = new File("/home/rohit/Desktop/Java_Scalar_Replacement/JavaScalarReplacement-main/src/main/java/example/");
            String srcFolder =  "/home/rohit/Desktop/Java_Scalar_Replacement/JavaScalarReplacement-main/src/main/java/example";
            ThreeACCode.findNonJavaFiles(new File((srcFolder)));

            try {
                SourceRoot root = new SourceRoot(Path.of(srcFolder));
                root.getParserConfiguration().setSymbolResolver(new JavaSymbolSolver(typeSolver));
                List<ParseResult<CompilationUnit>> parseResults = root.tryToParse();
                List<CompilationUnit> units = parseResults.stream()
                        .filter(ParseResult::isSuccessful)
                        .map(pr -> pr.getResult().get())
                        .collect(Collectors.toList());
                for (CompilationUnit cu : units) {
                    Path fileName = cu.getStorage().map(s -> s.getPath()).orElse(null);
                    System.out.println(fileName.getFileName());

                    // build classMap...
                    new ClassParameterVisitor().visit(cu, classMap);
                    new ClassTypeVisitor().visit(cu, null);

                    // points-to analysis...
                    PointsToAnalysis pta = new PointsToAnalysis(cu, classMap);
                    pointsToAnalysisGraphs = pta.pointsToAnalysis;

                    // scalar-replacement pass
                    MethodVisitor mv = new MethodVisitor();
                    mv.visit(cu, new ArrayList<>());
                    new FieldAccessVisitor().visit(cu, null);

                    // **insert the FieldAccessVisitor to rewrite b.value -> b_value**
                    new FieldAccessVisitor().visit(cu, null);

                    // write out
                    String target = fileName.toString().replace(srcFolder + "/", srcFolder + "_new/");
...
//                    MethodVisitor mv = new MethodVisitor();
//                    mv.visit(cu, objectNames);
//
//                    // ←– HERE: run the FieldAccessVisitor
//                    new FieldAccessVisitor().visit(cu, null);
//
//                    // 2) now dump out cu.toString()
//                    try (FileWriter w = new FileWriter(target)) {
//                        w.write(cu.toString());
            ...
            for (String root : roots) {
                ...
                for(CompilationUnit cu : compilations){
                    ...
                    new InterproceduralScalarVisitors.LocalVariableVisitor().visit(cu, null);
                    new FieldAccessVisitor().visit(cu, null);
                    new SignatureVisitor().visit(cu, null);
                    new CallVisitor().visit(cu, null);
...
// }
    ...
    private static class ClassTypeVisitor extends VoidVisitorAdapter<Void>{
        ...
        public void visit(ClassOrInterfaceDeclaration classOrInterfaceDeclaration, Void arg){
            if (!BasicScalarReplacement.staticTypes.containsKey(classOrInterfaceDeclaration.getName())) {
                return;
            }
        ...
        public void addDeclaration(NodeList<Statement> nodeList, String className, String parentClass, boolean addParentClass, String varName, Set<String> alreadyDeclared, Map<SimpleName, Type> localClassType){
            ...
            if(fieldDec.isPresent()){
                ...
                for (VariableDeclarator variableDeclarator : fieldDeclaration2.getVariables()){
                    ...
                    declaredScalars.add(newName);
//                    localClassType.put(variableDeclarator_new.getName(), variableDeclarator_new.getType());
//                    localClassType.put(localVar.toString(), varType.asString());
...
//    }
    ...
    private static class FieldAccessVisitor extends VoidVisitorAdapter<Void> {
        ...
        public void visit(FieldAccessExpr fae, Void arg) {
            ...
            String scalarName = ((NameExpr) scope).getName() + "_" + fae.getName();
            if (declaredScalars.contains(scalarName)) {
                fae.replace(new NameExpr(scalarName));
            }
        }
    }
    ...
    private static class ClassParameterVisitor extends VoidVisitorAdapter<Map<String, ClassOrInterfaceDeclaration>> {
        @Override
        public void visit(ClassOrInterfaceDeclaration c, Map<String, ClassOrInterfaceDeclaration> map) {
            super.visit(c, map);
            map.put(c.getNameAsString(), c);
            BasicScalarReplacement.staticTypes
                    .putIfAbsent(c.getName(), new HashMap<>());
